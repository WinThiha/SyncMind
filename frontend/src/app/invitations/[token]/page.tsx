...
export default function AcceptInvitePage({ params }: { params: Promise<{ token: string }> }) {
  ...
  useEffect(() => {
    async function loadInvite() {
      try {
        ...
        setFetchError(axiosError.response?.data?.message || 'This invitation link is invalid.');
      }
    }
    loadInvite();
  }, [token]);
  ...
  const handleAccept = async () => {
    ...
    try {
      ...
      const axiosError = err as AxiosError<{ message?: string }>;
      setFetchError(axiosError.response?.data?.message || 'Failed to accept invitation.');
    } finally {
      setAccepting(false);
    }
  };
      ...
      <div className="w-full sm:max-w-md">
        ...
        {fetchError && (
          <GlassCard className="p-10 text-center">
            ...
            <h2 className="text-xl font-black mb-3">
              {fetchStatus === 410 ? 'Invitation Expired' : 'Invalid Link'}
            </h2>
        ...
        {accepted && (
          <GlassCard className="p-10 text-center">
            ...
            <p className="text-foreground/50 text-sm font-medium">Redirecting to the project...</p>
          </GlassCard>
        )}
        ...
        {invite && !accepted && (
          <GlassCard className="p-10">
            <div className="text-center mb-8">
              ...
              <h2 className="text-2xl font-black mb-1">You&apos;re invited</h2>
              <p className="text-foreground/50 text-sm font-medium">
                {invite.inviter_name ? (
                  <><strong className="text-foreground/70">{invite.inviter_name}</strong> invited you to join</>
                ) : (
            ...
            <div className="bg-foreground/5 rounded-2xl border border-border-glow/30 p-6 mb-8 space-y-4">
              <div className="flex justify-between items-center">
                <span className="text-xs font-bold text-foreground/40 uppercase tracking-wider">Project</span>
            ...
            {user ? (
              ...
              >
                <CheckCircle size={18} />
                {accepting ? 'JOINING...' : `JOIN ${invite.project_name.toUpperCase()}`}
              </GlassButton>
            ) : (
