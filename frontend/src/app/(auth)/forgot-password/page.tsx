...
export default function ForgotPasswordPage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setError(axiosError.response?.data?.message || t('auth.forgotPassword.errorFallback'));
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <div className="min-h-screen bg-background flex flex-col justify-center py-12 px-4 sm:px-6 lg:px-8">
            <div className="sm:mx-auto sm:w-full sm:max-w-md text-center mb-10">
                <AppLogo size="lg" />
                <p className="text-foreground/40 font-medium mt-3">{t('auth.forgotPassword.subtitle')}</p>
            </div>
                ...
                <GlassCard className="p-10 shadow-2xl border-border-glow/50">
                    {sent ? (
                        ...
                        >
                            ...
                            <h2 className="text-xl font-black mb-3">{t('auth.forgotPassword.successTitle')}</h2>
                            <p className="text-foreground/50 text-sm font-medium mb-8">
                                {t('auth.forgotPassword.successDescriptionStart')} <strong className="text-foreground/70">{email}</strong> {t('auth.forgotPassword.successDescriptionEnd')}
                            </p>
                            <Link
                                href="/login"
                        ...
                        <>
                            <h2 className="text-2xl font-black text-center mb-2 tracking-tight">{t('auth.forgotPassword.heading')}</h2>
