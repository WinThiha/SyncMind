...
The system SHALL provide a toggle control in the sidebar header that switches between expanded (256px) and collapsed (64px) widths.

#### Scenario: User collapses the sidebar
- **WHEN** user clicks the chevron toggle button in the sidebar header
- **THEN** the sidebar transitions from 256px to 64px width and the chevron icon rotates 180 degrees

#### Scenario: User expands the sidebar
- **WHEN** the sidebar is collapsed and user clicks the chevron toggle button
- **THEN** the sidebar transitions from 64px to 256px width and the chevron icon rotates back to 0 degrees
...
When the sidebar is collapsed, each navigation icon SHALL display a native browser tooltip with the item's label text.

#### Scenario: Hover over collapsed navigation icon
- **WHEN** the sidebar is collapsed and user hovers over a navigation icon
- **THEN** a native browser tooltip appears showing the item's label (e.g., "Dashboard", "Settings", "Help")
...
- **WHEN** the sidebar is collapsed to 64px
- **THEN** the Topbar's left offset is 64px and the main content area's left margin is 64px
