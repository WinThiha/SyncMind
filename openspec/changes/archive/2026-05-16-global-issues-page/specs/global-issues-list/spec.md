...
The system SHALL provide a top-level Issues view accessible via the sidebar that displays all issues across all projects the user is a member of, with the ability to filter by project.
...
#### Scenario: User selects a specific project
- **WHEN** user selects a specific project from the project picker
- **THEN** the Issues page filters to show only issues from that project, and summary cards update to reflect only that project's data

#### Scenario: User selects "All projects"
- **WHEN** user selects "All projects" from the project picker
- **THEN** the Issues page displays issues from all projects, and summary cards reflect aggregate counts across all projects
